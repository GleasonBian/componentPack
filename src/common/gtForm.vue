  ...
  <el-card class="box-card">
    ...
    >
      ...
      >
        ...
        <!-- 日期选择器范围 -->
        <el-date-picker
          v-else-if="item.type === 'date-scope'"
          v-model="model[item.key]"
          type="daterange"
          format="yyyy年MM月dd日"
          ...
          style="width:100%;"
          @change="dateChange(model[item.key])"
          :style="item.width?'width:'+item.width+'%':'width:'+width+'%'"
        ></el-date-picker>
        ...
        <!-- 单图上传 -->
        <el-upload
          v-else-if="item.type === 'singleImg'"
          class="upload-img"
          :action="item.action?item.action: process.env.API_HOST+'/material/fileUpload/uploadImage'"
          :name="item.name?item.name:'imageFile'"
          :show-file-list="false"
          accept="image/*"
          :on-success="singleImageSuccess"
        ...
        >
          <img
            v-if="model[item.key]"
            :src="item.default?'http://file.sjgtw.com/'+item.default: 'http://file.sjgtw.com/'+model[item.key] "
            class="avatar"
          >
            ...
            >
              ...
              <el-table-column label="操作" width="50" align="center" header-align="center">
                <template slot-scope="scope">
                  <el-button
                    @click.native.prevent="deleteAreaRow(scope.$index, model[item.key])"
                    type="text"
                    size="small"
                  >删除</el-button>
                </template>
        ...
        >
          <el-tooltip
            :content="item.data.attrKey.content"
            placement="top"
            :value="tooltipKey"
            :manual="true"
          >
          ...
          <el-tooltip
            :content="item.data.attrVal.content"
            placement="top"
            :value="tooltipVal"
            :manual="true"
          >
  ...
  methods: {
    ...
    handleInputConfirm() {
      ...
      if (inputValue !== "") {
        ...
        } else this.$message.warning(this.inputValue + " 已存在");
      }
    ...
    areaChange(value) {
      ...
      if (newVal.length > 1 && newVal[newVal.length - 1] === "")
        newVal[newVal.length - 1] = "own";
      ...
      switch (item.type) {
        ...
        case "tag":
          if (
            ...
            Object.prototype.toString.call(item.default) === "[object String]"
          ) {
