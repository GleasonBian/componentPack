<template>
  <el-tabs type="border-card" v-model="activeName" :style="data.width?'width:'+data.width:'width:100%'">
    <el-tab-pane label="PC端" name="PC" :disabled="data.disabled=='PC' ? true:false">
      <el-upload class="avatar-uploader-PC" :action="data.action" :name="data.name?editor.name:'imageFile'"
        :show-file-list="false" :on-success="uploadSuccess" :on-error="uploadError" :before-upload="beforeUpload">
      </el-upload>
      <quill-editor v-model="editor[data.PC.key]" ref="QuillEditorPC" :options="editorOptionPC"
    ...
    <el-tab-pane label="MB端" name="MB" :disabled="data.disabled=='MB' ? true:false">
      <el-upload class="avatar-uploader-MB" :action="data.action" :name="editor.name?editor.name:'imageFile'"
        :show-file-list="false" :on-success="uploadSuccess" :on-error="uploadError" :before-upload="beforeUpload">
      </el-upload>
      <quill-editor v-model="editor[data.MB.key]" ref="QuillEditorMB" :options="editorOptionMB"
        @change="onEditorChange($event)" style="height:500px" v-loading='quillUpdateImg'>
      </quill-editor>
    </el-tab-pane>
  </el-tabs>
  ...
  export default {
    ...
    created() {
      ...
      if(editorProps.disabled === 'PC'){
        this.$set(this.editor,editorProps.MB.key,editorProps.MB.value?editorProps.MB.value:'');
      }else if(editorProps.disabled === 'MB'){
        this.$set(this.editor,editorProps.PC.key,editorProps.PC.value?editorProps.PC.value:'');
      }else{
        this.$set(this.editor,editorProps.MB.key,editorProps.MB.value?editorProps.MB.value:'');
        this.$set(this.editor,editorProps.PC.key,editorProps.PC.value?editorProps.PC.value:'');
      }
