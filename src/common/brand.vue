<template>
  ...
  <div>
    ...
    <el-dialog title="选择品牌" :visible.sync="isShowbrandBox" top="5vh" :center="true" custom-class="el-dialog"
      width="480px">
      ...
      <el-table :data="brandData" :border="true" fit :show-header="true" :highlight-current-row="true" size="small"
      ...
      @current-change="offsetChange" :current-page.sync="offset"
      :page-size="limit"
 >
      </el-pagination>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      async clickChooseBrand() {
        ...
        res.rows.map(item => {
          if(item.fileVO){
            ...
            item.fileVO = 'http://file.sjgtw.com/M00/04/38/wKgBxFp5d22EHfuBAAAAAHo6-30319.jpg';
          }
        })
      ...
      handleCurrentChange(row) {
        if(!row) return;
        this.$set(this.brand, "brandName", row.chineseName?row.chineseName:'');
