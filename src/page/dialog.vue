...
<script>
  ...
  export default {
     ...
     */
    created() {
      this.$bus.on('dialog-close',this.parentCloseDialog);
      this.$bus.on('submit',this.formChange)
    },
    mounted() {
      
    },
    beforeDestroy(){
      this.$bus.off('dialog-close',this.parentCloseDialog)
    }
  }
</script>
<style scoped>
